...
Files matching nothing become `unclassified`. `classification.ts` also exports the authored-script predicates (`SCRIPT_SOURCE_EXTENSIONS` / `isScriptSourceName` / `isCompiledSibling`) that both script-enumerating surfaces use to admit `.ts`/`.js` while suppressing a `.js` compiled-sibling — see ADR 0016.
