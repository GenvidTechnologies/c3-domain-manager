...
- `decisions/0029-txid-rejection-diagnostics.md` — adopt mcp-utils 0.10.0's discriminated `parseTxToken` to diagnose a malformed `txId` distinctly from a stale one, appending rather than replacing the wrong-project case, with `compareTxToken` left as the sole accept/reject authority (issue #79)
- `decisions/0030-classification-coverage-gate.md` — add an opt-in `--max-unclassified` threshold to `generate` (and `maxUnclassified` to the `regenerate` tool), establishing exit code 2 for a tripped gate while 1 stays "the command failed", generating the index before gating so the failure's own instructions survive, and scoping the `isError` bend to `regenerate` alone (issue #81)
