...
- **`sources[]`** — §5.1. `resource` is **required** within each entry. `id`
  ...
  **Do not cite `CLAUDE.md` as a source.** It is rewritten continuously, so a
  page citing it cannot be re-verified against a fixed text — the one property
  `sources[]` exists to guarantee. A claim worth a wiki page and found only in
  `CLAUDE.md` needs an ADR or an issue behind it first.
...
**This project's rule.** Judge by what would have to change for the page to
become wrong:

- A page whose claims are **measurements or accepted ADR decisions** — a
  platform behaviour, a decision record's reasoning — gets `stale_after` a
  year out. The measurement does not decay; only the reader's assumption that
  nobody has re-measured does.
- A page describing an **evolving practice**, or one whose claims are pinned
  to a dependency version, gets a few months. Dependency floors here move
  without anything in this repo detecting it, so a page that names a version
  is stale the moment upstream ships past it — that is the shortest fuse on
  the wiki and the reason this bucket exists separately.
- Omit the key only when the page would stay true even if every version,
  tool, and platform in it were replaced.
