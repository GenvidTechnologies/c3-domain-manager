...
> labels (`bug`, `enhancement`, `documentation`, `duplicate`, `question`,
> `wontfix`) plus `triaged`, with **no** `type:`/`priority/`/`area:` scheme.
>
> The commands below use **GitHub Issues via the `gh` CLI**. The section headings
> must stay the same — the skill and analyst locate guidance by heading.
